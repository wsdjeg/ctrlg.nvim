...
help:
	...
	@echo "  make test PATTERN=display               # Match test/**/*display*_spec.lua"
...
#   make test PATTERN=display  (shorthand for test/**/*display*_spec.lua)
test: install-deps
...
clean:
	...
	@rm -rf test/*.out
	@rm -rf *.swp
	@rm -rf /tmp/ctrlg_nvim_test_* 2>/dev/null || true
